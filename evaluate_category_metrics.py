...
EVISCREEN_ROOT = Path(__file__).resolve().parent
PROJECT_ROOT = EVISCREEN_ROOT.parent
DEFAULT_DATA_ROOT = EVISCREEN_ROOT / "stage_0_data_preparation" / "fundus_csv"
DEFAULT_EVAL_DIR = EVISCREEN_ROOT / "reproduce_directly" / "eval"

METRIC_NAMES = ["AUROC", "AP", "Pat95R", "Pat99R", "Pat100R", "CSR"]

DATASET_CONFIGS = {
    "JSIEC_original": {
        "csv_relpath": Path("fundus") / "JSIEC" / "test_original.csv",
    ...
    "RIADD_original": {
        "csv_relpath": Path("fundus") / "RIADD" / "test_original.csv",
        "path_token": "RIADD",
        "categories": [
            "DR",
            "ARMD",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize EviScreen evaluation outputs with category-wise metrics.")
    parser.add_argument(
        "--data-root",
        type=Path,
        default=DEFAULT_DATA_ROOT,
        help="Stage 0 fundus CSV root. Defaults to stage_0_data_preparation/fundus_csv.",
    )
...
def label_csv_path(split_name: str, data_root: Path) -> Path:
    return data_root / DATASET_CONFIGS[split_name]["csv_relpath"]


def load_labels(split_name: str, data_root: Path) -> pd.DataFrame:
    config = DATASET_CONFIGS[split_name]
    csv_path = label_csv_path(split_name, data_root)
...
def merge_predictions_with_labels(split_name: str, pred_path: Path, data_root: Path) -> tuple[pd.DataFrame, str]:
    pred_df, score_source_column = load_predictions(split_name, pred_path)
    label_df = load_labels(split_name, data_root)
    ...
    if len(merged) != len(pred_df):
        ...
        raise RuntimeError(
            f"{pred_path} could not be aligned with {label_csv_path(split_name, data_root)}. "
...
def load_results_file(split_name: str, results_path: Path, data_root: Path) -> tuple[pd.DataFrame, str]:
    label_df = load_labels(split_name, data_root)
    ...
    if len(results_df) != len(label_df):
        raise RuntimeError(
            f"Row count mismatch for {results_path} and {label_csv_path(split_name, data_root)}: "
...
def find_prediction_files(eval_dir: Path) -> list[tuple[str, Path]]:
    flat_files = []
    for pred_path in sorted(eval_dir.glob("*_predictions.csv")):
        split_name = pred_path.name[: -len("_predictions.csv")]
        flat_files.append((split_name, pred_path))

    nested_files = []
    for split_name in sorted(DATASET_CONFIGS):
        pred_path = eval_dir / split_name / "predictions.csv"
        if pred_path.exists():
            nested_files.append((split_name, pred_path))

    return flat_files + nested_files


def evaluate_split(split_name: str, pred_path: Path, data_root: Path) -> dict:
    merged, score_source_column = merge_predictions_with_labels(split_name, pred_path, data_root)
...
def main() -> None:
    ...
    if not (data_root / "fundus").exists():
        raise SystemExit(f"Fundus CSV root not found: {data_root / 'fundus'}")
    ...
    if args.results_file is not None:
        ...
        frame, score_source_column = load_results_file(split_name, results_file, data_root)
    ...
    pred_files = find_prediction_files(eval_dir)
    if not pred_files:
        raise SystemExit(f"No prediction files found in {eval_dir}")

    summary["eval_dir"] = str(eval_dir)

    for split_name, pred_path in pred_files:
        if split_name not in DATASET_CONFIGS:
            print(f"Skipping unknown split file: {pred_path.name}")
            continue

        split_result = evaluate_split(split_name, pred_path, data_root)
        split_result["prediction_file"] = str(pred_path)
        summary["splits"][split_name] = split_result

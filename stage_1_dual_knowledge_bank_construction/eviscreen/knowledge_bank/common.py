...
class NetworkFeatureAggregator(torch.nn.Module):
    ...
    def __init__(self, backbone, layers_to_extract_from, device):
        ...
        for extract_layer in layers_to_extract_from:
            ...
            if "." in extract_layer:
                if len(extract_layer.split(".")) == 3:
                    extract_block, extract_chunk, extract_idx = extract_layer.split(".")
                    network_layer = backbone.__dict__["_modules"][extract_block]
                    network_layer = network_layer[int(extract_chunk)]
    ...
    def forward(self, images):
        ...
        if self.backbone.name == "vit_base" or self.backbone.name == "vit_large" or self.backbone.name == "vit_large_patch14_dinov2meh" or self.backbone.name == "vit_large_patch14_dinov2" or self.backbone.name=="vit_large_patch14_dinov2_518" or self.backbone.name=="vit_large_patch14_dinov2meh_336" or self.backbone.name=="vit_large_panderm" or self.backbone.name=="vit_large_512" or self.backbone.name=="vit_large_512": 
            for key, value in self.outputs.items():
                value = value[:, 1:, :]
                if self.backbone.name=="vit_large_512":
                    value = value[:, 4:, :]
                B, L, C = value.shape
                H = W = int(L**0.5)
...
class NearestNeighbourScorer(object):
    def __init__(self, n_nearest_neighbours: int, nn_method=FaissNN(False, 4)) -> None:
        ...
        # self.imagelevel_nn = lambda query: 
    ...
    def predict(
        ...
        query_distances, query_nns = self.nn_method.run(self.n_nearest_neighbours, query_features)
        # query_distances = np.sqrt(query_distances)
